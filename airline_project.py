...
#          Includes analytics using Pandas and NumPy.
# ---------------------------------------------------------------
...
# ---- Step 1: File setup ----
# Always store booking data safely inside Documents/SkyFly_Data
BOOKING_FOLDER = os.path.join(os.path.expanduser("~"), "Documents", "SkyFly_Data")
os.makedirs(BOOKING_FOLDER, exist_ok=True)
BOOKING_FILE = os.path.join(BOOKING_FOLDER, "bookings.txt")
...
def print_banner():
    """Prints main heading."""
    print("\n" + "=" * 70)
    print("SKYFLY AIRLINES – AIRLINE BOOKING SYSTEM".center(70))
    print("=" * 70)


def section_title(title):
    """Prints section title with lines."""
    print("\n" + "-" * 70)
...
def search_flights():
    """Search flights by city (from or to)."""
...
# ---- Step 6: Booking functions ----
def book_ticket(bookings):
    """Book a ticket and save details."""
...
def create_fare_receipt_pdf(booking):
    """Generate a PDF receipt inside Documents/Flight_Receipts."""
...
def cancel_booking(bookings):
    ...
    for b in bookings:
        if b["name"].lower() == name.lower() and b["flight_no"] == flight_no:
            confirm = input(f"Confirm cancellation for {b.get('booking_id', 'N/A')}? (y/n): ").lower()
            if confirm == 'y':
                bookings.remove(b)
                save_bookings(bookings)
